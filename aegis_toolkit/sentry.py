...
async def inspect_request(request: Request, body: bytes, settings: Settings):
    ...
    """
    Inspects an incoming request against all Sentry (WAF) rules.
    """
    ...
    for rule in settings.sentry_rules:
        if rule.pattern:
            ...
            if "body" in rule.inspect_locations:
                if pattern.search(decoded_body_str):
                    _trigger_violation(rule, "request body")
            if "query_params" in rule.inspect_locations:
                if pattern.search(decoded_query_str):
